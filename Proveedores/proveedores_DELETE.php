...
if (!$isAjax) {
    ...
    header("Location: proveedores_READ.php");
    exit();
}
...
try {
    ...
    $check = $conn->prepare("SELECT id FROM proveedores WHERE id = ? AND estado = 1");
    $check->bind_param("i", $id);
    $check->execute();
    $check->store_result();
    
    if ($check->num_rows === 0) {
        echo json_encode(['success' => false, 'message' => 'El proveedor no existe o ya fue eliminado']);
        exit();
    }
    
    // Eliminar el proveedor (cambiar estado a 0)
