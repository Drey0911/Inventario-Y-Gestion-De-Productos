...
// Usar Prepared Statements para seleccionar el producto
$sql = "SELECT * FROM productos WHERE id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $id);
$stmt->execute();
...
// Verificar datos duplicados al realizar update
...
if ($resultado_verificacion->num_rows > 0) {
 ...
 if ($alerta === ""){
    // Usar Prepared Statements para la actualización
    $sql_update = "UPDATE productos SET nombre = ?, stock = ?, precio_unitario = ?, descripcion = ? WHERE id = ?";
    $stmt_update = $conn->prepare($sql_update);
    $stmt_update->bind_param("sidsi", $nombre, $stock, $precio_unitario, $desc, $id);
            ...
            <div class="col-lg-6 col-md-10 col-sm-12"> <!-- tamaño -->
                <form method="post" action="" class="mt-1 p-4 border rounded bg-light">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control" id="stock" name="stock" placeholder="Ingrese El Stock" value="<?php echo isset($user['stock']) ? htmlspecialchars($user['stock']) : ''; ?>" required>
                    </div>
                    <div class="form-group">
                        <label for="precio_unitario">Precio Unitario:</label>
                        <input type="number" class="form-control" id="precio_unitario" name="precio_unitario" placeholder="Ingrese El Precio Unitario" value="<?php echo isset($user['precio_unitario']) ? htmlspecialchars($user['precio_unitario']) : ''; ?>" required>
                    </div>
                    <div class="form-group">
                        <label for="desc">Descripcion:</label>
                        <input type="text" class="form-control" id="desc" name="desc" placeholder="Ingrese La Descripcion" value="<?php echo isset($user['descripcion']) ? htmlspecialchars($user['descripcion']) : ''; ?>" required>
                        <br>
